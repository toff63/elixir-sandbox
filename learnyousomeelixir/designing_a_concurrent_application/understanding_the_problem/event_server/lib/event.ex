defmodule Reminder.Event do
  ...
  To test just run
      ...
      ied> parent = self()
      #PID<0.115.0>
      iex> spawn fn -> Reminder.Event.loop(%Reminder.Event.State{server: parent, name: "test", to_go: 5}) end
      #PID<0.106.0>
      iex> flush()
      :ok
      iex> flush()
      {:done, "test"}
      :ok
      iex> pid = spawn fn -> Reminder.Event.loop(%Reminder.Event.State{server: parent, name: "test", to_go: 500}) end
      #PID<0.122.0>
      ...
      :ok
      iex> pid = spawn fn -> Reminder.Event.loop(%Reminder.Event.State{server: parent, name: "test", to_go: 365*24*60*60}) end
      #PID<0.127.0>
      iex>
      22:06:50.136 [error] Process #PID<0.127.0> raised an exception
      ** (ErlangError) Erlang error: :timeout_value
          (reminder_event) lib/reminder_event.ex:37: Reminder.Event.loop/1
